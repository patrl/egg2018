...
\subsection{Re-binding and exceptional scope}

\ex
I want to know who left my party early and what was bothering him,\newline
and \textsc{Bill} wants to know Δ too.
\xe
    
\ex Context: \emph{both I and Mary had at least one unsatisfied guest at our respective parties.}\newline
I want to know \(\begin{aligned}[t]
&\text{[who\(^{x}\) left my party early]}\\
\text{and }&\text{[what was bothering him\(_{x}\)]}
\end{aligned}\),\newline
and \textsc{Mary} wants to know \(\begin{aligned}[t]
&\text{[who\(^{x}\) \elide{left her party early}]}\\
\text{and }&\text{[what \elide{was bothering him\(_{x}\)}]}
\end{aligned}\)
\xe
...
\xe
  ...
  \item Now let's define a unary type-lifter \(\eta\), that shifts ordinary values to inhabitants of \(\type{F}\), and a binary operation \(⍟\) for composing inhabitants of \(\type{F}\).\sidenote{\(⟨\type{F},η,⍟⟩\) constitutes an \emph{applicative functor}. This guarantees that is is \emph{composable} with other applicative functors, providing us with assurance that our theory of focus will play nicely with other applicative machinery.

    See, e.g., \citet{charlow2018} for an account of assignment-sensitivity using \emph{applicative functors}.
}
...
\ex
    \(\metalang{foc}\,x ≔ (x, \Set{y|\metalang{alt}_{x}\,y} )\)
\xe
    
\ex
\begin{forest}
[{\((\metalang{left}\,\metalang{Bill}, \Set{\metalang{left}\,x| \metalang{alt}_{{B}}\,x})\)\\\(\type{F\,t}\)\\\( ⍟\)}  [{\((\metalang{Bill},\Set{x|\metalang{alt}_{{B}}\,x})\)\\\(\type{F\,e}\)\\\(\metalang{Bill}_{\metalang{foc}}\)}]
  [{\((\metalang{left}, \Set{\metalang{left}} )\)\\\type{F\,(e \rightarrow t)}\\\(\metalang{left}^{\eta}\)}]]
\end{forest}
\xe
    
\ex
I know who \textsc{John} will kiss,\newline
and also who \textsc{Mary} will \elide{kiss}
\xe
...
\begin{forest}
    [{...}
      ...
      [{...}
        ...
        [{.}
          [{Mary}]
          [{\(λ g . \metalang{kiss} g₂\)},tikz={\node [draw,dashed,fit=()(!1)(!2)] {};}
            [{kiss}]
            [{\(t_{2}\)}]]]]]
\end{forest}
    
    \[λ g . \metalang{kiss} g₁ ∉ \Set{λ g . \metalang{kiss} g₂}\]\marginnote{Since \(λ g . \metalang{kiss} g₁ ≢ λg . \metalang{kiss}\,g_{2}\)}
    
\ex
\label{rebindingTree2}I know who \textsc{John} will kiss,\newline
and also who \textsc{Mary} will \elide{kiss}    
\xe
 ...
 \begin{forest}
  [{...}
   [{who}]
   [{\(λ g . λ x . \metalang{kiss} g₁^{[1→x]} \metalang{John}\)},tikz={\node [draw,red,fit=()(!1)(!lll)] {};}
     [{\(Λ_{1}\)}]
     [{\ldots}
       [{John}]
       [{\ldots}
...
\begin{forest}
    [{...}
      ...
      [{\(\left(
          \begin{aligned}[c]
          &λ g . λ x . \metalang{kiss} g₂^{[2 → x]} \metalang{Mary},\\
          &\Set{λ g . λ x . \metalang{kiss} g₂^{[2→x]} y | \metalang{alt}_{M} y}
          \end{aligned}
        \right)\)},tikz={\node [draw,dashed,fit=()(!1)(!lll)] {};}
        [{\(Λ_{2}\)}]
        [{\ldots}
          [{Mary\(_{F}\)}]
          [{\ldots}
